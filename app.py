...
@app.get("/test_battle", response_class=HTMLResponse)
async def test_battle(request: Request):
    """
    Generates and saves a demo HTML battle for testing.
    """
    demo_html = """
    <html>
    <head><title>Demo Battle Replay</title></head>
    <body style="background:#101010; color:#fff; text-align:center; font-family:sans-serif;">
      <h1>⚔️ Demo Battle Replay</h1>
      <p>This is a sample battle page generated by /test_battle</p>
      <img src="/static/battle_placeholder.png" alt="Battle Preview" style="width:300px; border-radius:10px;">
      <p>Battle completed successfully!</p>
    </body>
    </html>
    """

    path = "battles/demo_battle.html"
    with open(path, "w", encoding="utf-8") as f:
        f.write(demo_html)
